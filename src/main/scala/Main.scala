...
object Main extends ZIOAppDefault {
  ...
  def run: ZIO[Any, Throwable, Unit] =
    ...
    for {
    _ <- console.putStrLn("Please enter a file: ")
    fileName <- console.getStrLn
    path <- ZIO.fromOption(Paths.get(fileName)).orElseFail("Invalid file path")
    jsonStr <- readFile(path)
    sudokuGrid <- parseJson(jsonStr)
    _ <- console.putStrLn(s"Successfully parsed Sudoku grid: $sudokuGrid")
    } yield ()

    program.catchAllCause(cause => console.putStrLn(s"Error: ${cause.prettyPrint}"))
}
